...
int display(string path, string word_to_search) {
    ...
    if (myfile.is_open()) {
        ...
        myfile.close(); // close the file after use
    }
    else {
        cout << "File not open\n" << endl;
    }
...
int main() {
    string Path = "C:/xampp/htdocs/parallel/"; // use forward slash instead of backslash in the path
    ...
    for (int i = 1; i <= 5; i++) {
        ...
        cout << "Total Count is " + to_string(display(Path + npath, word_to_search)) << " from file " + to_string(i) << endl;
    }
